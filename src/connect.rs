...
impl PoolConnection {
    pub fn new(username: &str, address: &str, worker_name: &str) -> Self
    {
        ...
        return PoolConnection {
            ...
            workername: worker_name.to_owned(),
            address: address.to_owned(),
            active_job_queue: vec![],
            stream: stream.try_clone().expect("Unable to clone TcpStream"),
            reader: BufReader::new(stream)
        };
    }

    pub fn submit_share(&mut self, job_id: &String, extra_nonce2: Vec<u8>, ntime: &Vec<u8>, nonce: u32) {
        let extra_nonce2_str = &hex::encode(extra_nonce2);
        let ntime_str = &hex::encode(ntime);
        let nonce_str = format!("{:x}", nonce);
        let message = r#"{"id": 4, "method": "mining.submit","params":[""#.to_owned() + &self.username + "." + &self.workername + r#"",""# + &job_id + r#"",""# + &extra_nonce2_str + r#"",""# + &ntime_str + r#"",""# + &nonce_str + r#""]}"#;
    ...
    pub fn init_connection<'b>(&mut self) {
        self.subscribe_pool();
        self.authorize_pool();
    }

    pub fn handle_datastream(&mut self) -> Option<Job> {
